...
class ClientThread(Thread):
	def  __init__(self,ip,port):
		...
		self.ip=ip
		self.port=port
		print ("[+] New hotel server socket thread started for "+ ip +":"+str(port))
...
threads=[]
def check_host(host):
	if "www.hotel.com" in host:
		return  1
	else: return 0

while True:
	tcpServer.listen(4)
	print("multithreaded hotel server : waiting for conncetions from http clients...")
	...
	while True:
		try:
			...
			if check_host(string):
				print("hereeeeeeeee")
				p =1
			#check_available_room(string)
			else: break
			print("hotel Client Says: " + string)
